...
class InvoiceApp {
    ...
    async printInvoice(invoiceId) {
        ...
        // Scale A4 content to fit the screen width on mobile/small screens
        const scaleBillPreview = () => {
            ...
            const contentWidth = contentEl.scrollWidth || 794; // A4 ~794px
            if (panelWidth < contentWidth + 40) {
                const scale = (panelWidth - 16) / contentWidth;
                contentEl.style.transform = `scale(${scale})`;
                contentEl.style.transformOrigin = 'top center';
                contentEl.style.marginBottom = `${(scale - 1) * contentEl.offsetHeight}px`;
            } else {
                contentEl.style.transform = '';
                contentEl.style.transformBottom = '';
            }
        };
