...
class InvoiceApp {
    ...
    async printInvoice(invoiceId) {
        ...
        // Helper: generate PDF blob from bill preview content
        const generatePdfBlob = async () => {
            // Temporarily set A4 width for PDF capture
            const contentEl = previewContent.firstElementChild;
            const prevWidth = contentEl ? contentEl.style.width : '';
            const prevTransform = contentEl ? contentEl.style.transform : '';
            if (contentEl) {
                contentEl.style.width = '794px';
                contentEl.style.transform = 'none';
            }
            const canvas = await html2canvas(previewContent.firstElementChild, { scale: 2, useCORS: true });
            if (contentEl) {
        ...
        newDownloadBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                // --- MOBILE / PWA DOWNLOAD (saves to device Downloads folder) ---
                // Try the modern File System Access API first (Android WebView / Chrome)
                if (window.showSaveFilePicker) {
                    try {
                        ...
                        this.showNotification('Invoice saved to your device!', 'success');
                        newDownloadBtn.disabled = false;
                        newDownloadBtn.innerHTML = '<i class="fas fa-download"></i> Download PDF';
                        return;
                    } catch (fsErr) {
                        // User cancelled or API unavailable — fall through to anchor download
                    }
                }

                // Fallback: standard anchor download (works on all browsers & WebIntoApp)
                ...
                this.showNotification('Invoice downloaded to your Downloads folder!', 'success');
            } catch (err) {
        ...
        newShareBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                // Use Web Share API (works on Android/iOS Chrome, WebIntoApp, etc.)
                if (navigator.share && navigator.canShare && navigator.canShare({ files: [file] })) {
                    ...
                    // Share API available but file sharing not supported — share URL/text instead
                    const url = URL.createObjectURL(blob);
                    // Download first, then share link fallback
                    const a = document.createElement('a');
                    a.href = url;
                    a.download = fileName;
                    document.body.appendChild(a);
                    ...
                    await navigator.share({
                        title: `Invoice ${invoice.invoiceNumber}`,
                        text: `Invoice ${invoice.invoiceNumber} from ${settings.businessName || 'Nema Trader'}. The PDF has been saved to your Downloads.`
                    });
                    ...
                    a.click();
                    setTimeout(() => { document.body.removeChild(a); URL.revokeObjectURL(url); }, 1000);
                    this.showNotification('PDF downloaded! You can share it from your Downloads folder.', 'info');
                }
            } catch (err) {
        ...
        // Show the preview panel (hide nothing else — panel is fixed overlay)
        previewPanel.style.display = 'block';
        previewPanel.scrollTop = 0;
        // Store which page to return to
        this._previewReturnPage = this.currentPage;

        // --- RESPONSIVE BILL SCALING ---
        // On mobile: scale A4 bill to fit screen width
        const scaleBillPreview = () => {
            const contentEl = previewContent.firstElementChild;
            if (!contentEl) return;
            const wrapper = document.getElementById('bill-preview-scroll-wrapper');
            const availW = (wrapper ? wrapper.clientWidth : window.innerWidth) - 16;
            // A4 content is ~794px wide
            const billW = 794;
            if (availW < billW) {
                ...
                contentEl.style.marginBottom = `${(scale - 1) * contentEl.offsetHeight}px`;
                // Adjust wrapper height so page doesn't cut off
                previewContent.style.width = billW + 'px';
                previewContent.style.height = (contentEl.offsetHeight * scale) + 'px';
        ...
        setTimeout(scaleBillPreview, 80);
        window.addEventListener('resize', scaleBillPreview);
        // Clean up resize listener when preview is closed
        this._scaleBillPreviewFn = scaleBillPreview;
    }
    ...
    async printQuotation(quotationId) {
        ...
        newDownloadBtn.addEventListener('click', async () => {
            ...
            this.showNotification('Creating PDF, please wait...', 'info');
            try {
                // Temporarily restore full A4 width for capture
                const contentEl = previewContent.firstElementChild;
                const prevWidth = contentEl ? contentEl.style.width : '';
                const prevTransform = contentEl ? contentEl.style.transform : '';
                if (contentEl) { contentEl.style.width = '794px'; contentEl.style.transform = 'none'; }
                const canvas = await html2canvas(previewContent.firstElementChild, { scale: 2, useCORS: true });
                if (contentEl) { contentEl.style.width = prevWidth; contentEl.style.transform = prevTransform; }
                const imgData = canvas.toDataURL('image/png');
                const { jsPDF } = window.jspdf;
                const pdf = new jsPDF('p', 'mm', 'a4');
                const pdfWidth  = pdf.internal.pageSize.getWidth();
        ...
        // Responsive scaling for quotation bill
        const scaleQuotationPreview = () => {
            const contentEl = previewContent.firstElementChild;
            if (!contentEl) return;
            const wrapper = document.getElementById('bill-preview-scroll-wrapper');
            const availW = (wrapper ? wrapper.clientWidth : window.innerWidth) - 16;
        ...
        setTimeout(scaleQuotationPreview, 80);
        window.addEventListener('resize', scaleQuotationPreview);
        this._scaleBillPreviewFn = scaleQuotationPreview;
    }
